...
import datetime
import json
from sklearn.feature_extraction.text import TfidfVectorizer  # Importa TfidfVectorizer
...
class SentimentAnalyzer:
    def __init__(self):
        ...
        self.tfidf_vectorizer = TfidfVectorizer()  # Inicializa TfidfVectorizer

    def create_experiment_directory(self):
        """Crea un nuevo directorio para el experimento actual."""
        base_dir = 'experiments_tfidf'
    ...
    def get_bert_embedding(self, text):
        ...
        print("Obteniendo representación vectorial...")        
        ...
        for idx, row in df.iterrows():
            for q_num, column_name in column_mapping.items():
                if column_name in df.columns:
                    ...
                    if processed_text:
                        all_processed_texts.append(processed_text)  # Agregar a la lista
                        embedding = self.get_bert_embedding(processed_text)
                        X.append(embedding[0])
                        y.append(sentiment_mapping[q_num])

        # Ajustar TfidfVectorizer a todos los textos procesados
        print("Ajustando TfidfVectorizer...")
        self.tfidf_vectorizer.fit(all_processed_texts)

        # Transformar los textos procesados en vectores TF-IDF
        print("Transformando textos a vectores TF-IDF...")
        X_tfidf = self.tfidf_vectorizer.transform(all_processed_texts)

        # Convertir X_tfidf a una matriz NumPy densa
        X_tfidf = X_tfidf.toarray()

        # Concatenar los embeddings de BERT con los vectores TF-IDF
        X = np.concatenate((X, X_tfidf), axis=1)
    ...
    def predict_sentiment(self, text):
        ...
        # Obtener embedding de BERT
        embedding = self.get_bert_embedding(processed_text)

        # Obtener vector TF-IDF
        tfidf_vector = self.tfidf_vectorizer.transform([processed_text]).toarray()

        # Concatenar embedding de BERT con vector TF-IDF
        final_embedding = np.concatenate((embedding, tfidf_vector), axis=1)

        prediction = self.classifier.predict(final_embedding)
        probabilities = self.classifier.predict_proba(final_embedding)
...
def main():
    ...
    components = {
        ...
        'tfidf_vectorizer': analyzer.tfidf_vectorizer  # Incluir tfidf_vectorizer en el diccionario
    }
    ...
    for text in texts:
        sentiment, probs = analyzer.predict_sentiment(text)  # Llamar al método actualizado
